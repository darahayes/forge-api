...
var host = '46.101.92.165';
var port = 3000;
var url = `http://${host}:${port}`;

// request({method: 'POST', url: `${url}/auth/register`, json: true, body: {email: 'dara@example.com', password: 'password'}}, function(err,response,body) {
//   console.log(err);
...
request({method: 'POST', url: `${url}/auth/login`, json: true, body: {email: 'dara@example.com', password: 'password'}}, function(err,response,body) {
  console.log(err);
  console.log(response.headers['content-type']);
  ...
  console.log(login);
  request({method: 'POST', url:`${url}/api/exercises`, json: true, body: {"name": "my_pushup", "category": "Resistance", "tags": ["Chest", "Arms"]}}, function(err,response,body){ 
      console.log(response.headers['content-type']);
      console.log(err)
      console.log(body)
  });
